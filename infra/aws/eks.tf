...
resource "aws_eks_node_group" "eksng02" {
  cluster_name    = aws_eks_cluster.eks01.name
  node_group_name = "${var.prefix}-${var.environment}-eksng02"
  node_role_arn   = aws_iam_role.iamr02.arn
  ...
  launch_template {
    name    = aws_launch_template.lt02.name
    version = "$Latest"
  }

  labels = tomap({
    "node.roiergasias" = "node2"
  })
...
resource "aws_launch_template" "lt02" {
  name          = "${var.prefix}-${var.environment}-lt02"
...
## NOTE: commented out fargate profiles as it does not support node labels but instead schedules based on pod labels
...
# resource "aws_eks_fargate_profile" "eksfp02" {
#   cluster_name           = aws_eks_cluster.eks01.name
#   fargate_profile_name   = "${var.prefix}-${var.environment}-eksfp02"
#   pod_execution_role_arn = aws_iam_role.iamr03.arn
#   subnet_ids             = [module.vpc.private_subnets[1]]
